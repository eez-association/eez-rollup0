    ...
    <L2 as TransactionsProvider>::Transaction: Encodable2718,
{
    ...
    /// Sealed header at `l2_block`, erroring if neither the DB nor the
    /// committer's in-memory canonical head holds it. Routes through
    /// `sealed_header_or_head` so main's reorg/forkchoice/cursor-alignment
    /// helpers (the callers below) see a freshly-derived, not-yet-persisted
    /// catch-up head and don't spuriously deadlock the L1-derived follower.
    fn l2_sealed_header_at(
        &self,
        l2_block: u64,
    ) -> DeriverResult<SealedHeader<alloy_consensus::Header>> {
        self.sealed_header_or_head(l2_block)?.ok_or_else(|| {
