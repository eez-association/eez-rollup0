...
//!
//! Window/resume contract: the prover is composer-driven — it takes each verify
//! range (`from_block .. to_block`) from the composer's `ProverDispatch` stream
//! (the oldest posted-but-unverified window), subscribes at `from_block`, and
//! accumulates until the settling `to_block`. A reconnect re-receives the SAME
//! oldest-unverified window and replays it from `from_block` (the feed's
//! ring-buffer replay + strictly-increasing block numbers make this gap-free); a
//! durable checkpoint lets a wide directive (spanning > `max_window`) resume
//! mid-range. Intra-window contiguity (parent_hash + number) is checked as events
//! arrive.
//!
...
fn driven_effective_settlement(
    ...
    if Some(event.block_number) == target_to_block && event.composition.is_none() {
    ...
    if Some(event.block_number) != target_to_block {
        warn!(
            block_number = event.block_number,
            ?target_to_block,
...
async fn main() -> eyre::Result<()> {
    ...
    if signer.is_none() {
        warn!(
            "no signer key — a prover that does not ATTEST cannot advance the composer's verified \
             frontier, so the composer will re-dispatch the same window forever. Set \
             EEZ_PROOF_SIGNER_KEY to attest."
        );
    }
    ...
    // Log-correlation only — the composer derives WHAT to dispatch from its own
    // posted/attested ledger, never from a prover-asserted height.
    ...
    loop {
        ...
        // RECEIVE one directive (the oldest posted-but-unverified window) before
        ...
        let driven_resume = load_driven_checkpoint(&args.work_dir, &vr).await;

        // from_block comes from the directive (= posted+1, the OD-5 anchor block
        // + 1), unless the same directive has a durable non-settling checkpoint —
        // in which case resume at proved_to+1 with the checkpointed telescope state
        // below.
        let from_block = match &driven_resume {
            Some(resume) => resume.proved_to.saturating_add(1),
            None => vr.from_block,
        };
        ...
        // highest block number proved in THIS connection. It only anchors
        // intra-directive contiguity (backfill gap detection), never window
        // SELECTION. Seeded to `from_block - 1` so the FIRST event (block
        // `from_block` = posted+1, the directive) passes the gap check directly and
        // only a TRUE ring-eviction (event > from_block) triggers backfill. Seeding
        // to `posted` keeps the whole posted batch self-contained from its
        // boundary — the OD-5 anchor is re-derived from block `from_block`'s own
        // witness, never a [1..posted] re-verify from genesis.
        ...
        loop {
            ...
            match next_event {
                Ok(Some(mut event)) => {
                    // Never verify past the directive's to_block. By construction
                    // to_block is the settling block (is_sync) and the window closes
                    // + breaks there, so this fires only if that block carried NO
                    ...
                    if event.block_number > vr.to_block
                        ...
                        )
                    {
                        error!(
                            ...
                            to_block = vr.to_block,
                            "streamed past the directive to_block without a settling composition — refusing + re-requesting",
                        );
                        consecutive_failures += 1;
                        break;
                    }
                    ...
                    if window.is_empty() && event.block_number != stream_tip + 1 {
                        ...
                        match &l2_rpc_url {
                            ...
                            Some(url) if got > expected => {
                                ...
                                let replay_upper = got.min(vr.to_block.saturating_add(1));
                    ...
                    if is_sync || window.len() >= args.max_window {
                        ...
                        if (validating || vkey_configured) && !window_ok {
                            // The composer dictated this window; on ANY gate rejection
                            // (including an OD-5 anchor mismatch) refuse to attest +
                            // reconnect to re-request. The composer re-dispatches the SAME
                            // oldest-unverified window (it never attested), so the verified
                            // frontier physically CANNOT advance past a bad window —
                            // fail-closed, no false attestation. A transient stale-anchor
                            // off-by-one heals when the composer re-dictates `from_block`
                            // from its L1-confirmed cursor on the next dispatch; a genuine
                            // composer bug surfaces as a loud reconnect-backoff livelock,
                            // never a silent skip. A pure observer (no validator, no vkey)
                            // never sets `window_ok = false`, so it advances + moves on.
                            error!(
                                window_end,
                                "window rejected by a gate — refusing to attest; reconnecting to re-request the directive"
                            );
                            consecutive_failures += 1;
                            break;
                        }
                        ...
                        if let (Some(signer), Some(hash)) = (&signer, attest_hash) {
                            // The prover signs its OWN recomputed hash, but it MUST equal
                            // the directive's publicInputsHash — that is the key the
                            // composer's window is recorded under (mark_attested matches the
                            ...
                            if vr.public_inputs_hash.as_slice() != hash.as_slice() {
                                error!(
                                    recomputed = %hash,
                                    "recomputed publicInputsHash != directive hint — attestation will NOT match the composer's window; the frontier cannot advance (signing the recomputed hash regardless)",
                                );
                            }
                        ...
                        // Advance the in-session streaming tip (the per-chunk contiguity
                        // anchor). A non-settling chunk close only moves this tip; window
                        // SELECTION always comes from the directive, so a restart
                        // mid-directive re-streams from `posted+1` (or the durable
                        // checkpoint) and re-derives the OD-5 batch anchor (`state(posted)`)
                        // by re-executing the batch's first chunk.
                        ...
                        if validating && !is_sync {
                            if let (Some(anchor), Some(prev_final), Some(prev_hash)) = (
                                ...
                                .await
                                {
                                    warn!(
                                        error = %e,
                                        from_block = vr.from_block,
                                        to_block = vr.to_block,
                                        proved_to = window_end,
                                        "failed to checkpoint accepted replay progress",
                                    );
                                }
                            }
                        }
                        if is_sync {
                            // The composer dictated this frontier. The directive is
                            // satisfied iff this settling block IS its to_block; break to
                            // receive the next directive. A settling composition before
                            // to_block cannot occur within one posted batch (it settles only
                            // at sync_height) — guard fail-closed.
                            if window_end == vr.to_block {
                                if proof_sink_accepted {
                                    clear_driven_checkpoint(&args.work_dir).await;
                                    info!(
                                        window_end,
                                        "directive settled + attested — awaiting the next directive",
                                    );
                                } else {
                                    warn!(
                                        window_end,
                                        "settling block verified but ProofSink did not accept an attestation; retaining checkpoint for retry, awaiting re-dispatch",
                                    );
                                }
                                break;
                            }
                            error!(
                                window_end,
                                to_block = vr.to_block,
                                "settling composition at a block != directive to_block (impossible for one posted batch) — refusing + re-requesting",
                            );
                            consecutive_failures += 1;
                            break;
                        }
...
mod tests {
    ...
    fn driven_settlement_ignores_non_target_postbatch_sidecar() {
        ...
        assert!(!driven_effective_settlement(&mut event, Some(20), None));
        assert!(
