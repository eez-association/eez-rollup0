...
fn parse_args<const N: usize>(args: [&str; N]) -> Result<Args, clap::Error> {
    ...
    if !args.contains(&"--attester-address") {
        args.extend(["--attester-address", TEST_ATTESTER_ADDRESS]);
    }
...
#[test]
fn signer_key_must_match_the_expected_attester() {
    let args = parse_args([
        "eez-proof-signer",
        "--chain-config",
        "chain-config.json",
...
#[test]
fn native_system_configuration_requires_no_private_key_and_rejects_other_addresses() {
    let args = parse_args([
        "eez-proof-signer",
        "--chain-config",
        "chain-config.json",
    ...
    .unwrap();
    assert_eq!(
        Config::from_args(args).unwrap().expected_l2_system_address,
        eez_primitives::SYSTEM_ADDRESS
    );
    let args = parse_args([
        "eez-proof-signer",
        "--chain-config",
    ...
    assert!(
        !Args::command()
            .get_arguments()
            .any(|arg| arg.get_id() == "system_transaction_key")
    );
}
...
#[test]
fn attester_address_is_mandatory_and_must_match_the_signing_key() {
    assert!(
        parse_exact_args([
            "eez-proof-signer",
            "--chain-config",
