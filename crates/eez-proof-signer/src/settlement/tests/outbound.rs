...
fn outbound_events_bind_by_transaction_and_preserve_duplicate_hashes() {
    ...
    batch.inner.entries[1].l2ToL1Calls[0].data = Bytes::from_static(&[0x01]);
    ...
    *settling.outbound_event_candidates_mut_for_test() = vec![
    ...
    let call = &duplicate_batch.inner.entries[1].l2ToL1Calls[0];
    ...
    *reordered.outbound_event_candidates_mut_for_test() = vec![
        observed_outbound_call(1, 0, &batch.inner.entries[2].l2ToL1Calls[0]),
        observed_outbound_call(3, 0, &batch.inner.entries[1].l2ToL1Calls[0]),
    ];
...
fn outbound_bindings_require_a_preceding_load_and_outbound_first_order() {
    ...
    missing_load
        ...
        .push(observed_outbound_call(
            0,
            0,
            &outbound_batch.inner.entries[1].l2ToL1Calls[0],
        ));
    ...
    inbound_first
        ...
        .push(observed_outbound_call(
            2,
            0,
            &mixed_batch.inner.entries[2].l2ToL1Calls[0],
        ));
...
fn outbound_events_reject_missing_extra_multiple_and_malformed_observations() {
    let outbound_batch = effect_batch(&[B256::ZERO; 3], &[ClaimedEntryShape::Outbound]);
    let call = &outbound_batch.inner.entries[1].l2ToL1Calls[0];
...
fn outbound_events_bind_the_single_call_and_expected_rollup() {
    ...
    let observation = observed_outbound_call(1, 0, &valid.inner.entries[1].l2ToL1Calls[0]);
    ...
    multiple_calls.inner.entries[1]
        .l2ToL1Calls
        .push(l2_to_l1_call());
    assert_eq!(
    ...
    wrong_destination.inner.entries[1].destinationRollupId = U256::from(2);
    assert_eq!(
    ...
    wrong_source.inner.entries[1].l2ToL1Calls[0].sourceRollupId = U256::from(2);
    assert_eq!(
        verify(&wrong_source),
    ...
    for mutate in hash_mutations {
        ...
        mutate(&mut wrong_call.inner.entries[1].l2ToL1Calls[0]);
        assert!(matches!(
            verify(&wrong_call),
...
fn outbound_effects_require_a_successful_outcome_and_exact_value_accounting() {
    ...
    nonempty_return_data.inner.entries[1].returnData = Bytes::from_static(&[0xca, 0xfe]);
    nonempty_return_data.inner.entries[1].rollingHash =
    ...
    wrong_return_data.inner.entries[1].returnData = Bytes::from_static(&[0xca, 0xfe]);
    assert_eq!(
    ...
    failed.inner.entries[1].returnData = Bytes::from_static(&[0xca, 0xfe]);
    failed.inner.entries[1].rollingHash =
        b256!("f01319463ceddff2696a5a9252bb370015c485dd3526b2f0302c2681c3d5c71a");
    failed.inner.entries[1].stateDeltas[0].etherDelta = I256::ZERO;
    assert_eq!(
        verify(&failed),
        Some(OutboundEffectError::UnsupportedOutcome { entry_index: 1 })
    );
    ...
    failed_with_value.inner.entries[1].l2ToL1Calls[0].value = value;
    failed_with_value.inner.entries[1].stateDeltas[0].etherDelta = -I256::try_from(value).unwrap();
    assert_eq!(
    ...
    wrong_delta.inner.entries[1].stateDeltas[0].etherDelta = I256::ONE;
    assert_eq!(
        verify(&wrong_delta),
    ...
    for value in [U256::from(1), (U256::from(1) << 255) - U256::from(1)] {
        ...
        value_bearing.inner.entries[1].l2ToL1Calls[0].value = value;
        value_bearing.inner.entries[1].stateDeltas[0].etherDelta = -I256::try_from(value).unwrap();
        assert_eq!(verify(&value_bearing), None);

        value_bearing.inner.entries[1].stateDeltas[0].etherDelta += I256::ONE;
        assert_eq!(
    ...
    for value in [U256::from(1) << 255, U256::MAX] {
        ...
        out_of_range.inner.entries[1].l2ToL1Calls[0].value = value;
        assert_eq!(
            verify(&out_of_range),
    ...
    reserved_source.inner.entries[1].l2ToL1Calls[0].sourceAddress = SYSTEM_ADDRESS;
    assert_eq!(
