...
| `--l2-system-key` | `EEZ_L2_SYSTEM_KEY` | none | Valid secp256k1 key deriving the reserved `SYSTEM_ADDRESS`. |
...
The attestation key MUST NOT derive `SYSTEM_ADDRESS`; attestation and reserved
L2 transaction construction are separate authorities. The configured system
key MUST derive `SYSTEM_ADDRESS` exactly.
...
The active deployment constants are:
...
The signer does not verify deployed code at `EEZL2_ADDRESS`. Matching code and
constructor configuration are deployment invariants.
...
```text
signer       = operator-provided key for SYSTEM_ADDRESS
