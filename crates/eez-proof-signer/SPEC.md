...
| --- | --- |
| Operator configuration | Authoritative for execution-chain rules, expected rollup ID, proof-system address and vkey, attester identity, reserved system address, and resource limits. The deployment MUST use the fixed EEZL2 predeploy and reserved native system address. |
...
| `--l2-system-address` | `EEZ_L2_SYSTEM_ADDRESS` | none | Address embedded in EEZL2; MUST equal reserved native sender `0xeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee0076`. |
...
attestation and system-transaction construction are separate authorities.
System-transaction construction uses no private key.
...
The signer verifies that `EEZ_L2_SYSTEM_ADDRESS` equals the reserved constant. It
...
4. recover Ethereum transaction signers using the configured fork rules, including
   the Homestead low-`s` rule when active, and assign the fixed sender to native
   system transactions.
