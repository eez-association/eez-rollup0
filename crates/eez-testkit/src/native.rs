...
pub async fn spawn_composer(world: &CrossChainWorld, builder_url: &str) -> Result<MirrorComposer> {
    ...
    let [l1_http, l1_auth, l2_xchain, l1_p2p, l1_discv5, l1_xchain] =
        port_leases.each_ref().map(PortLease::port);
    ...
    for (name, value) in [
        ...
        ("EEZ_L1_P2P_PORT", l1_p2p.to_string()),
        ("EEZ_L1_DISCV5_PORT", l1_discv5.to_string()),
        ...
        ("EEZ_L1_XCHAIN_PORT", l1_xchain.to_string()),
        ...
        (
            ...
            format!("{:#x}", signer_address(world.cfg.attester_key)?),
        ),
        (
            "EEZ_WITNESS_DB_PATH",
...
async fn mirror_l1(source: &str, destination: &str, jwt: JwtSecret) -> Result<()> {
    ...
    loop {
        ...
        // The Prague fixture has an empty requests list. V4 preserves its hash;
        // V3 drops it and makes the mirrored block's header hash differ.
        ensure!(
            block.header.requests_hash == Some(alloy_eips::eip7685::EMPTY_REQUESTS_HASH),
            "L1 mirror fixture expects empty execution requests at block {number}"
        );
...
impl SelectedBuilder {
    pub async fn start() -> Result<Self> {
        ...
        let server = ServerBuilder::default()
            .build((std::net::Ipv4Addr::LOCALHOST, 0))
            .await?;
