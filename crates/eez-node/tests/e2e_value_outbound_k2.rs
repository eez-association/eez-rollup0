...
/// K>=2 ON-CHAIN: TWO value-bearing outbound withdrawals (anvil#3 + anvil#4,
/// both -> the SAME L1 target proxy, back-to-back so they DRAIN INTO ONE SLOT)
/// settle as a single K=2 batch, and a fresh follower re-derives it from L1
/// alone with the A4 gate (HARD) authorizing BOTH outbound immediates. Proves
/// the L1 contract consumes 2 outbound immediates + the 2-link state-delta
/// chain, and the deriver pairs+gates 2 entries — the on-chain mirror of the
/// unit `outbound_gate_k2_multiple_immediates_one_slot`. Mirrors
/// `e2e_value_outbound` (deposit-to-fund the reserve, then withdraw), doubled.
#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
async fn value_outbound_k2_two_withdrawals_settle_and_follower_rederives() {
    ...
    // ── S3b — TWO WITHDRAWALS (value-bearing outbound), K=2 in ONE slot. anvil#3
    // and anvil#5 each attach WITHDRAW_WEI on L2 to the SAME outbound proxy,
    // submitted BACK-TO-BACK (send_outbound just submits, no mine-wait) so both
    // land in the held pool before the composer's next drain → ONE K=2 batch.
    // EEZL2 burns M per call to SYSTEM_ADDRESS; on L1 the rollup pays 2*M from its
    // reserve to the L1 target (two outbound immediates, etherDelta=-M each). Both
    // are L2-ingress-FRESH (anvil#4, the inbound depositor, already consumed an
    // ingress nonce, so it can't be reused here — hence anvil#5).
    let total_out = U256::from(WITHDRAW_WEI) * U256::from(2u64);
    let wd_hash_1 = send_outbound_set_value(
        &l2_rpc_b,
        ANVIL_KEY_3,
        proxy_out,
        OUTBOUND_VALUE,
    ...
    .await
    .expect("submit WITHDRAW #1 (anvil#3) to L2 ingress");
    // Withdrawal #2 uses a DISTINCT setValue arg (OUTBOUND_VALUE+1) so the two
    // L2->L1 calls are unambiguously different (defends against any same-calldata
    // dedup collapsing them to one). The L1 target's final value is whichever
    // settles last; the K=2 proof is the BALANCE (2*M), not the value.
    let outbound_value_2 = OUTBOUND_VALUE + 1;
    let wd_hash_2 = send_outbound_set_value(
        &l2_rpc_b,
        ANVIL_KEY_5,
        proxy_out,
        outbound_value_2,
    ...
    .await
    .expect("submit WITHDRAW #2 (anvil#5) to L2 ingress");
    eprintln!(
        "[S3b] TWO WITHDRAWALS submitted (K=2): {WITHDRAW_WEI} wei each to proxy_out={proxy_out:#x} \
         #1={wd_hash_1:#x} (setValue {OUTBOUND_VALUE}) #2={wd_hash_2:#x} (setValue {outbound_value_2})"
    );

    // ── S3b acceptance (a) — the L1 target received 2*M (BOTH withdrawals paid),
    // its value is one of the two setValue args, AND the reserve was debited 2*M.
    ...
    assert!(
        withdrawn.is_ok(),
        "TWO WITHDRAWALS did not both pay out: L1 target ETH balance = {l1_final_bal} (expected \
         2*M = {total_out}), L1 target value = {l1_final_val} (expected {OUTBOUND_VALUE} or {outbound_value_2}). \
         One of the two outbound immediates was not consumed (K=2 settlement / etherDelta=-M per call / L2 burn).",
    );
    assert_eq!(
        reserve_after,
        U256::from(FUND_WEI) - total_out,
        "reserve must be debited by exactly 2*M ({FUND_WEI} - 2*{WITHDRAW_WEI}), got {reserve_after}",
    );
    eprintln!(
        "[S3b] ACCEPTANCE (a): L1 target balance == {l1_final_bal} (== 2*M), value == {l1_final_val}, \
         rollup etherBalance == {reserve_after} (== V - 2*M) ✓"
    );

    // ── S3b acceptance (c) — BOTH WITHDRAW user txs' own L2 receipts are SUCCESS
    // (K=2 burn pin). Each tx burns M to SYSTEM_ADDRESS (EEZL2.sol:192-195)
    // BEFORE its `_consumeAndExecute`, so two status=1 receipts prove both
    // burns committed (total 2*M actually left L2, matching the 2*M reserve
    // debit on L1). Before the lean build_l2_outbound_entry shape each consume
    // reverted `RollingHashMismatch` (EEZL2.sol:422), rolling back the burns —
    // L1 would pay 2*M while the L2 senders kept 2*M (silent inflation that the
    // L1-side acceptance (a)/(b) never caught).
    for (label, hash) in [("#1", wd_hash_1), ("#2", wd_hash_2)] {
        let status = wait_for_l2_tx_receipt(&l2_rpc_b, hash, SETTLE_TIMEOUT)
            .await
            .unwrap_or_else(|e| panic!("WITHDRAW {label} {hash:#x} never got an L2 receipt: {e}"));
        assert!(
            status,
            "WITHDRAW {label} user tx {hash:#x} reverted on L2 (status=0) — expected SUCCESS. \
             Its L2 burn of M to SYSTEM_ADDRESS did not commit (RollingHashMismatch from a \
    ...
    let reconciled = wait_for(SETTLE_TIMEOUT, || async {
        ...
        // Gate on BOTH withdrawals having paid out (L1 target balance == 2*M) so
        // we pin a post-K=2-withdraw root, not an earlier deposit/anchor/partial.
        let l1_bal = eth_get_balance(&l1.rpc_url, l1_value_addr).await?;
        Ok(match l2 {
            Some((num, root))
    ...
    // K=2 EVIDENCE: a SINGLE reconcile line with outbound>=2 — the two withdrawals
    // drained into ONE batch (not two K=1 batches). Back-to-back submission with no
    // mine-wait puts both held entries in the pool before the composer's
    // (seconds-long, L1-bound) settle cycle, so they batch deterministically.
    let k2_one_slot = log.lines().any(|l| {
        l.contains("built outbound load + inbound delivery system txs")
            && field_at_least(l, "outbound=", 2)
    });
    eprintln!(
        "[S4] follower deriver markers: reconcile(cross_chain)={reconcile_ran} \
         ...
         withdraw(outbound>0)={built_outbound} K=2-one-slot(outbound>=2)={k2_one_slot}"
    );
    assert!(
    ...
    assert!(
        k2_one_slot,
        "expected a K=2 batch (a 'built ...' reconcile line with outbound>=2) — the two \
         back-to-back withdrawals should drain into ONE slot. If they split into two K=1 \
         batches the deterministic balance/reserve assertions still hold but the K=2 path is \
         untested; inspect {}.",
        follower.log_path.display(),
    );
...
/// True iff `line` contains `field` followed by an integer `>= min` (e.g.
/// `outbound=2` for the K=2 batch evidence).
fn field_at_least(line: &str, field: &str, min: u64) -> bool {
    line.split(field).skip(1).any(|rest| {
        let digits: String = rest.chars().take_while(char::is_ascii_digit).collect();
        digits.parse::<u64>().is_ok_and(|v| v >= min)
    })
}
