//! Inbound (L1→L2) cross-chain E2E — the MIRROR of `e2e_outbound.rs`.
//!
//! Topology (mirror of the outbound test):
//!
//! | | OUTBOUND (`e2e_outbound.rs`) | INBOUND (this file) |
//! |---|---|---|
//! | `Value` target | L1 | **L2** |
//! | CrossChainProxy | L2 (`createCrossChainProxy(L1Value, MAINNET=0)`) | **L1** (`createCrossChainProxy(L2Value, L2_rollupId)`) |
//! | user tx chain-id | L2 (=1) | **L1** (embedded reth `--dev` = 1337) |
//! | user tx `to` | L2 proxy | **L1 proxy** |
//! | classifier signal | `EEZ_CROSS_CHAIN_PROXY_ADDRESSES` (L2 proxy) | **`EEZ_CROSS_CHAIN_SOURCE_CHAIN_IDS`** (L1 chain id) |
//! | submitted to | L2 ingress | L2 ingress (same) |
//! | settlement effect | L1 `Value.value() == 42` | **L2 `Value.value() == 42`** |
//!
//! The cross-chain composer + deriver already support inbound (it is the
//! ORIGINAL design — `IngressClassifier` matches inbound by
//! `tx.chain_id ∈ cross_chain_source_chain_ids`, the composer drains it
//! via `simulate_and_resolve` → `build_inbound_system_txs`
//! (`executeIncomingCrossChainCall`), and the deriver reconstructs the
//! inbound delivery system txs in `reconcile_batch_blocks`). This test is
//! pure harness wiring: stand up the SAME embedded-L1 composer, deploy the
//! `Value` target on **L2** + the CrossChainProxy on **L1**, fire the
//! L1-signed `setValue(42)` at the L2 ingress, assert it settles on L2,
//! and prove a fresh follower re-derives the settled L2 root from L1 alone.
//!
//! Bring-up (Phase A placeholder-then-Phase B restart) + the follower
//! demotion are byte-for-byte the same as `e2e_outbound.rs`; see that
//! file for the deploy-ordering / persistence-window / Mode::Follower
...
/// The INBOUND user tx sets the L2 `Value` to this.
...
async fn inbound_set_value_settles_on_l2_and_follower_rederives() {
    ...
    eprintln!("[S2] L2 Value deployed @ {l2_value_addr:#x}  value()={v0}");
    ...
    .await
    .expect("submit inbound setValue(42) to L2 ingress");
    eprintln!(
        "[S3] inbound user tx submitted: chain_id={l1_chain_id} (L1) to={p_l1:#x} \
         data=setValue({INBOUND_VALUE}) hash={tx_hash:#x}"
    );
    ...
    eprintln!("[S3] ACCEPTANCE (a): L2 Value.value() == {final_value} ✓");
