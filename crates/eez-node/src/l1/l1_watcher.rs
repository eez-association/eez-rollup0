...
use crate::l1::config::{parse_address, parse_u64, parse_url};
use crate::l1::error::{L1Error, L1Result};
use crate::l1::scan::ScannedBatch;
...
pub struct L1WatcherConfig {
    ...
    /// [`SubmitterConfig::rpc_url`](crate::l1::SubmitterConfig::rpc_url).
    pub rpc_url: Url,
            ...
            Some((old_tip_number, old_tip_hash)) => {
                ...
                if latest_number > old_tip_number.saturating_add(depth) {
                    ...
                    let chunk_to = scan_from
                        .saturating_add(crate::l1::scan::LOG_SCAN_CHUNK_BLOCKS - 1)
                        .min(latest_number);
                    ...
                    let scanned = crate::l1::scan::scan_batch_logs_range(
                        provider,
