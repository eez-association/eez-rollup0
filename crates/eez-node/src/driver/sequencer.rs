...
use crate::driver::block_committer::BlockCommitterHandle;
use crate::driver::error::{DriverError, DriverResult};
use crate::driver::slot::{SlotEvent, SlotKind, SyncSlotComposerHandle, SyncSlotMode};
use crate::driver::timing::{RollupTiming, SlotComposition};
...
struct SpeculativeLimit {
    ...
    source: Arc<crate::l1::L1CanonicalHead>,
}
...
/// then `newPayload` via the [`BlockCommitter`](crate::driver::BlockCommitterHandle).
        ...
        + 'static,
{
    ...
    /// [`MAX_BLOCKS_PER_CATCHUP`](crate::driver::MAX_BLOCKS_PER_CATCHUP) per
    ...
    async fn advance_live_tick(&mut self, target_wall: u64) -> DriverResult<()> {
        ...
        while produced < crate::driver::MAX_BLOCKS_PER_CATCHUP {
        ...
        if produced == crate::driver::MAX_BLOCKS_PER_CATCHUP {
            event!(
    ...
    fn sync_block_misses_l1_slot(&self, sync_slot_timestamp: u64) -> bool {
        ...
        let ready_ms = crate::driver::slot::now_unix_millis().saturating_add(proof_ms);
    ...
    async fn advance_sync_slot(
        ...
        let comp = self.timing.per_trigger_composition(
            head,
            sync_slot_block_height,
            crate::driver::MAX_BLOCKS_PER_CATCHUP,
        );
    ...
    async fn commit_one_prebuilt(
        &mut self,
        kind: SlotKind,
        built: crate::driver::slot::SyncSlotBlock,
    ) -> DriverResult<()> {
