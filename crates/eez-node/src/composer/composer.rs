...
//!   Drains the rollup's [`HeldPool`](crate::composer::HeldPool), runs every held
...
//!   on failure (see [`crate::composer::optimistic`]).
...
//!   writer of [`L1CanonicalHead`](crate::l1::L1CanonicalHead)).
//!
//! Each batch is proved by the shared [`Prover`] and sent via the shared
//! [`Submitter`](crate::l1::Submitter) bundle relay.
...
use crate::composer::held_pool::HeldTx;
use crate::composer::ingress::Direction;
use crate::composer::local::{build_sync_block, sync_block_pair_roots};
use crate::composer::optimistic::OptimisticallyIncluded;
use crate::composer::rollup::RollupState;
...
pub struct CrossChainExecCtx {
    ...
    /// reth / anvil).
    pub submitter: crate::l1::Submitter,
    ...
    <L2 as TransactionsProvider>::Transaction: Encodable2718,
{
    ...
    async fn compose_sync_slot(
        ...
        // on a stale parent (see [`crate::driver::ParentContext`]).
    ...
    <L2 as TransactionsProvider>::Transaction: Encodable2718,
{
    ...
    async fn recover_failed_batch(
        ...
        failed: crate::composer::optimistic::FailedBatch,
    ) -> Option<SyncSlotBlock> {
        ...
        // simulation.
        if let Some(pool) = rollup.held_pool.as_ref() {
            let mut keep: Vec<crate::composer::HeldTx> = Vec::with_capacity(failed.txs.len());
            let mut release: Vec<crate::composer::HeldTx> = Vec::new();
    ...
    async fn compose_via_evm_composer(
        ...
        // observed by a background task (see [`crate::composer::optimistic`]).
