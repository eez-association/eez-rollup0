#!/usr/bin/env bash
# Verify that rbuilder honors bundle timestamp bounds.
set -euo pipefail
...
for t in cast curl; do
    command -v "$t" >/dev/null || { echo "builder timestamp check: $t not found" >&2; exit 1; }
done

ADDR="$(cast wallet address --private-key "$KEY")"
bal="$(cast balance "$ADDR" --rpc-url "$RPC")"
[[ "$bal" != "0" ]] || { echo "builder timestamp check: probe $ADDR has 0 balance" >&2; exit 1; }

head="$(cast block-number --rpc-url "$RPC")"
if (( head < 130 )); then
    echo "builder timestamp check: WARN L1 at block $head (< ~4 epochs). The flashbots relay"
...
# Return 0 when included, 1 when excluded, and 2 on infrastructure failure.
probe() {
...
echo "== builder timestamp check =="
echo "probe=$ADDR rpc=$RPC builder=$BUILDER"
...
# Surface rbuilder's rejection reason when logs are available.
if command -v kurtosis >/dev/null 2>&1; then
    blogs="$(kurtosis service logs "${KURTOSIS_ENCLAVE:-eez-ci}" \
        "${KURTOSIS_BUILDER_SERVICE:-el-2-reth-builder-lighthouse}" 2>/dev/null || true)"
