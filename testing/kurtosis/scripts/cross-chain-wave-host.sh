...
# The rig runs a prebuilt image, so a result only counts if the binary matches
# the tree. One shared symbol proves nothing — a stale image keeps every old
# name. Require EVERY event name the tree's submitter/composer declare.
if [[ "${EEZ_SKIP_IMAGE_CHECK:-0}" != "1" ]]; then
    ...
    if docker cp eez-node-kurtosis:/usr/local/bin/eez-composer "$_bin" >/dev/null 2>&1; then
        ...
        echo "    ✓ running image carries every event name in the working tree"
    fi
fi
echo "    L1 (shared)  = $L1"
echo "    L2           = $L2"
...
refresh_node_log() { docker logs "$(docker ps --format "{{.Names}}" | grep -m1 "^eez-node-kurtosis$")" >"$NODE_LOG" 2>&1 || true; }
refresh_signer_log() { docker logs "$(docker ps --format "{{.Names}}" | grep -m1 "^eez-proof-signer-kurtosis$")" >"$SIGNER_LOG" 2>&1 || true; }
...
# The node logs JSON (`"settled":true`); a terminal-format node logs `settled=true`.
settled_count() { refresh_node_log; strip_ansi <"$NODE_LOG" | grep -cE 'settled(=|":)true' || true; }
...
run_waves() {
    ...
    LAST_SETTLED=$(strip_ansi <"$NODE_LOG" | grep "bundle outcome observed" | grep -E 'settled(=|":)true' \
    ...
    if [[ -n "$LAST_SETTLED" ]]; then
        while (( SECONDS < root_deadline )); do
            ...
            # This branch commits the Sync block's HASH, not its state root.
        ...
        if (( root_matched )); then
            echo "    ✓ L1 rollups($EEZ_ROLLUP_ID).stateRoot == L2 safe block hash at height $L2_SAFE"
        else
            echo "    ✗ L1 stateRoot $L1_RECHECK != L2 safe block hash $L2_ROOT at height $L2_SAFE"; ok_all=0
        fi
    ...
    local DIVERGED
    # Stable event names, not message text: the deriver emits diverged_pre at
    # from_block-1 and diverged_post at to_block.
    DIVERGED=$(grep -cE 'eez\.deriver\.state\.diverged_(pre|post)' "$NODE_LOG" 2>/dev/null || true); DIVERGED=${DIVERGED:-0}
    if (( DIVERGED == 0 )); then
    ...
    refresh_node_log
    refresh_signer_log
    # Tolerate both log formats: JSON renders `"hash":"0x.."`, terminal `hash=0x..`.
    attested_hash=$(strip_ansi <"$NODE_LOG" | grep 'remote prover attested the window' \
        | grep -oE '"?hash"?[=:]"?0x[0-9a-fA-F]{64}' | tail -1 | grep -oE '0x[0-9a-fA-F]{64}' || true)
    if [[ -n "$attested_hash" ]]; then
        signer_line=$(strip_ansi <"$SIGNER_LOG" \
            | grep -E "recomputed_public_inputs_hash\"?[=:]\"?$attested_hash" | tail -1 || true)
    fi
