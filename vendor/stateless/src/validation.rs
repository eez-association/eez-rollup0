    ...
    E: ConfigureEvm<Primitives = N> + Clone + 'static,
{
    ...
    let (transaction_state_checkpoints, result) = {
        let mut executor = evm_config
            ...
            .map_err(|error| {
                StatelessValidationError::StatelessExecutionFailed(format!("{error:?}"))
            })?;
    ...
    Ok(StatelessValidationWithStateCheckpointsOutput {
        validation,
        checkpoints: BlockStateCheckpoints {
...
fn validate_transaction_indices(
    ...
    if let Some(indices) = transaction_indices
        .windows(2)
        .find(|indices| indices[0] >= indices[1])
    {
    ...
    if let Some(&index) = transaction_indices
        .last()
        .filter(|&&index| index >= transactions)
    {
    ...
    ChainSpec: Send + Sync + EthChainSpec<Header = Header> + EthereumHardforks + Debug,
{
    ...
    let block_access_list_hash = block_access_list
        .as_deref()
        .map(compute_block_access_list_hash);
