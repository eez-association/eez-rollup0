...
fn selected_transaction_checkpoints_match_the_recorded_full_state_roots() {
    ...
    assert_eq!(
        detailed.validation.post_state_root,
    ...
    assert_eq!(
        oracle.expected_checkpoint_indices.len(),
        oracle.transaction_state_roots.len()
    );
    let expected = oracle
        ...
        .map(
            |(state_root, transaction_index)| TransactionStateCheckpoint {
                transaction_index,
                state_root,
            },
        )
    ...
    assert_eq!(
        sparse.checkpoints.transaction_state_checkpoints,
