...
- `EEZ_L1_RPC_URL`        → upstream the `/rpc` proxy forwards to
...
- `EEZ_L2_RPC_URL`        → L2 node RPC, used to resolve real L2 block numbers
...
- **Batch payload → decode**: each row has a `decode` toggle that parses the
  ...
  `sync-rollups-protocol/out/EEZ.sol/EEZ.json` — run `forge build` if missing).
