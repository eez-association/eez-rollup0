...
def load_deployment_config():
    ...
    for k in ("EEZ_L1_RPC_URL", "EEZ_REGISTRY_ADDRESS", "EEZ_REGISTRY_DEPLOY_BLOCK"):
        if os.environ.get(k):
            cfg[k] = os.environ[k]
    return root, cfg
...
def main():
    ...
    upstream = cfg.get("EEZ_L1_RPC_URL")
    if not upstream:
        raise SystemExit(
            f"EEZ_L1_RPC_URL not found in {root}/.env or {root}/deployments.env "
            "(or environment). Cannot determine the L1 RPC."
        )
    abi = load_abi(root)
    # L2 RPC (optional) — lets the page resolve real L2 block numbers per batch.
    l2_upstream = cfg.get("EEZ_L2_RPC_URL", "http://127.0.0.1:18688")
